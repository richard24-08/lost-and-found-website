...
<body class="bg-gray-50 text-gray-900 font-sans h-full" id="app">
        ...
        <table class="min-w-full text-sm">
          ...
          <tbody id="reportsTbody" class="text-gray-700">
            <tr class="border-t">
              ...
              <td class="py-4 px-4"><span class="inline-block bg-green-100 text-green-700 px-3 py-1 rounded-full text-xs font-medium">found</span></td>
            </tr>
            <tr class="border-t">
              ...
              <td class="py-4 px-4"><span class="inline-block bg-red-100 text-red-700 px-3 py-1 rounded-full text-xs font-medium">lost</span></td>
            </tr>
          </tbody>
        </table>
      </div>
  ...
  <script>
    ...
    new Chart(ctxL, {
      ...
      data: {
        labels: lineLabels,
        datasets: [{ label: 'Reports', data: lineData, borderColor: '#06b6d4', tension: 0.3, fill: true }]
      },
      options: { responsive: true, maintainAspectRatio: false }
    });

    const ctxP = document.getElementById('pieChart').getContext('2d');
    const pie = new Chart(ctxP, {
      type: 'doughnut',
      data: { labels: pieLabels, datasets: [{ data: pieData, backgroundColor: ['#06b6d4','#ef4444','#8b5cf6','#f59e0b'] }] },
      options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } } }
    });

    const legend = document.getElementById('legend');
    pie.data.labels.forEach((label,i) => {
      legend.innerHTML += `<div class="mb-2">• <strong>${label}</strong>: ${pie.data.datasets[0].data[i]}</div>`;
    });
